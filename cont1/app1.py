...
def callback(ch, method, properties, body, channel):
    ...
    if message['cont'] == 'cont2' and message['target'] == 'cont1':
        data = int(message['message']['data']) + 1
        send_message(channel, "cont1", "app1.py", "counter", data, 'cont2')

def main():
    print("Скрипт запущен", file=sys.stderr)
    connection, channel = prepare_all()

    send_message(channel, "cont1", "app1.py", "counter", 0, 'cont2')
